...
surf_options = {
    ...
    "CL0": 0.951,  # CL of the surface at alpha=0
    "CD0": 0.007,  # CD of the surface at alpha=0
    # Airfoil properties for viscous drag calculation
    "k_lam": 0.05,  # percentage of chord with laminar
    # flow, used for viscous drag
    "t_over_c": 0.13670,  # thickness over chord ratio (FX 63137)
    "c_max_t": 0.35,  # chordwise location of maximum (FX 63137)
    # thickness
...
indep_var_comp.add_output("v", val=55.136, units="m/s")  # Freestream Velocity
indep_var_comp.add_output("alpha", val=0.0, units="deg")  # Angle of Attack
indep_var_comp.add_output("Mach_number", val=0.0)  # Freestream Mach number
indep_var_comp.add_output("re", val=1.2e6, units="1/m")  # Freestream Reynolds number
...
prob.driver = om.ScipyOptimizeDriver(maxiter=50)
prob.driver.options["debug_print"] = ["nl_cons", "objs", "desvars"]

recorder = om.SqliteRecorder("trial.db")
...
prob.model.add_constraint(point_name + ".Wing_perf.CL", equals=0.951)
